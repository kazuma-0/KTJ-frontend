...
function Tweets() {
  ...
  const prefetchTweets = useCallback(() => {
    ...
    fetchTweets(program).then((tweetItems) => {
  ...
  async function deleteTweet(pubKey) {
    ...
    await delete_tweet(program, wallet, pubKey)
      .then(() => {
        fetchTweets(program).then((tweetItems) => {
          const mapped = tweetItems.map((twts) => {
            return new Tweet(twts.publicKey, twts.account);
  ...
  return (
    <div className='mx-auto max-w-[calc(100%_-_500px)] max-h-[calc(100vh_-_110px)] h-[calc(100vh_-_150px)] flex'>
      <div className='p-5 px-10 border-r-2 h-full w-[300px] space-y-5'>
        <Button
          variant={'outline'}
          className='py-7 w-full shadow-lg'
          leftIcon={<IconHome />}
        >
          Home
        </Button>
        <Button className='py-7 w-full shadow-lg' leftIcon={<IconCloud />}>
          Topics
        </Button>
      </div>
      <div
        ...
        {!wallet.publicKey ? (
          ...
          <div className='px-5 py-5 bg-[#1C1A1E] rounded-lg shadow-xl outline-sky-400 ring-2 ring-white/75'>
            ...
            <Textarea
              ...
              value={content.text ?? ''}
              placeholder='Tweet your content! (max 300 chars)'
              resize={'auto'}
            ></Textarea>
